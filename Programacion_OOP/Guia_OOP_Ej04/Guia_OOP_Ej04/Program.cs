...
namespace Guia_OOP_Ej04
{
    ...
    La cantidad máxima de tinta para todos los bolígrafos será de 100. Generar una constante cantidadTintaMaxima en Boligrafo 
    donde se guardará dicho valor.
    ...
    Se deberá validar que el nivel de tinta resultante sea mayor o igual a cero y menor o igual a cantidadTintaMaxima. Si no es válido, 
    no se deberá modificar el atributo ni realizar ninguna acción.
    El método Recargar colocará la tinta en su nivel máximo. Reutilizar código.
    El método Pintar restará la tinta gastada (reutilizar código). El parámetro gasto representará la cantidad de unidades de tinta a 
    utilizar y utilizará tanta tinta como tenga disponible sin quedar en negativo. Utilizando el parámetro dibujo informará el 
    resultado retornando tantos * como unidades de tinta haya gastado, por ejemplo:
    ...
    En el método Main, crear un bolígrafo de tinta azul (ConsoleColor.Blue) y una cantidad inicial de tinta de 100 y 
    ...
    */
    ...
    internal class Program
    {
        ...
        {
            Console.Title = "Ejercicio OOP 04";

            string dibujo;
            
            Boligrafo bolAzul = new Boligrafo(100, ConsoleColor.Blue);

            Console.WriteLine(bolAzul.GetTinta());

            if (bolAzul.Pintar(30, out dibujo))
            {
            ...
            Console.WriteLine(bolRojo.GetTinta());
            bolRojo.Recargar();
            Console.WriteLine(bolRojo.GetTinta());

            Console.WriteLine();

            if(bolRojo.Pintar(101, out dibujo))
            {
                Console.WriteLine(dibujo);
            }
            ...
            if(!bolRojo.Pintar(10, out dibujo))
            {
                Console.WriteLine("No hay mas tinta!!");
            }
        }
    }
}
